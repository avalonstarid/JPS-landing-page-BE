<template>
  ...
  >
    ...
    <div class="card-group flex flex-col gap-4">
      <ElDivider class="!my-4">Section Header</ElDivider>
      ...
      <div class="gap-4 grid grid-cols-2">
        <div class="flex flex-col gap-4 p-4 border border-gray-200 rounded-lg">
          ...
          <InputBase
            v-if="generalSetting.header_title"
            v-model="generalSetting.header_title.id"
            :errors="errors"
            label="Title (ID)"
            name="header_title.id"
          />

          <InputBase
            v-if="generalSetting.header_desc"
            v-model="generalSetting.header_desc.id"
            :autosize="{ minRows: 2 }"
            :errors="errors"
            label="Deskripsi (ID)"
            name="header_desc.id"
        ...
        >
          ...
          <InputBase
            v-if="generalSetting.header_title"
            v-model="generalSetting.header_title.en"
            :errors="errors"
            label="Title (EN)"
          ...
          <InputBase
            v-if="generalSetting.header_desc"
            v-model="generalSetting.header_desc.en"
            :autosize="{ minRows: 2 }"
            :errors="errors"
            label="Deskripsi (EN)"
            name="header_desc.en"
...
const onSubmit = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      ...
      await storeLandingSetting('landing-karir', formData)
    }
  })
}

onMounted(() => {
  getGeneralSetting('landing_karir')
})

const copyFromId = (type: string) => {
  if (type === 'header') {
    generalSetting.value.header_desc!.en = generalSetting.value.header_desc!.id
    generalSetting.value.header_title!.en =
      generalSetting.value.header_title!.id
  } else if (type === 'hero') {
