<template>
  ...
  >
    ...
    <div class="card-group flex flex-col gap-4">
      <ElDivider class="!my-4">Section Lini Bisnis</ElDivider>
      ...
      <div class="gap-4 grid grid-cols-2">
        <div class="flex flex-col gap-4 p-4 border border-gray-200 rounded-lg">
          ...
          <InputBase
            v-if="generalSetting.business_line_title"
            v-model="generalSetting.business_line_title.id"
            :errors="errors"
            label="Title (ID)"
            name="business_line_title.id"
          />
        </div>
        ...
        >
          <div class="flex justify-between items-center">
            ...
            <button
              ...
              type="button"
              title="Copy from Indonesia"
              @click="copyFromId('business_line')"
            >
              <i class="ki-filled ki-copy"></i> Copy From ID
            </button>
          </div>
          ...
          <InputBase
            v-if="generalSetting.business_line_title"
            v-model="generalSetting.business_line_title.en"
            :errors="errors"
            label="Title (EN)"
            name="business_line_title.en"
          />
        </div>
      </div>
    </div>
    ...
    <div class="card-group flex flex-col gap-4">
      <ElDivider class="!my-4">Section CTA</ElDivider>
      ...
      <div class="gap-4 grid grid-cols-2">
        <div class="flex flex-col gap-4 p-4 border border-gray-200 rounded-lg">
          ...
          <InputBase
            v-if="generalSetting.cta_text"
            v-model="generalSetting.cta_text.id"
            :errors="errors"
            label="Text (ID)"
            name="cta_text.id"
          />
        ...
        >
          ...
          <InputBase
            v-if="generalSetting.cta_text"
            v-model="generalSetting.cta_text.en"
            :errors="errors"
            label="Text (EN)"
            name="cta_text.en"
          />
...
const onSubmit = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      ...
      await storeLandingSetting('landing-lini-bisnis', formData)
    }
  })
}

onMounted(() => {
  getGeneralSetting('landing_lini_bisnis')
})

const copyFromId = (type: string) => {
  if (type === 'business_line') {
    generalSetting.value.business_line!.en =
      generalSetting.value.business_line!.id
  } else if (type === 'cta') {
    generalSetting.value.cta_text!.en = generalSetting.value.cta_text!.id
  } else if (type === 'hero') {
