...
class BlogController extends Controller
{
	...
	{
		$data = Cache::remember('landing:blog', 3600, function () {
			$settings = Cache::rememberForever('settings:landing_blog', function () {
				return Setting::where('group', 'landing_blog')->get();
			...
			$seo = new SEOData(
				title: $settings['seo_title'] ?? 'Blog - ' . config('app.name'),
				description: $settings['seo_description'] ?? 'Artikel blog terbaru dari PT Janu Putra Sejahtera.',
	...
	{
		$data = Cache::remember("landing:blogDetail:$slug", 3600, function () use ($slug) {
